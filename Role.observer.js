...
StructureObserver.prototype.getAvailRoomsInRange = function () {
        if (!this.cache.avail)
                this.cache.avail = _.filter(this.getRoomsInRange(), rn =>
this.isValidObservationTarget(rn));
...
StructureObserver.prototype.isInRange = function (roomName) {
        return Game.map.getRoomLinearDistance(this.pos.roomName, roomName, false) <=
this.getRange();
};
/**
 ...
 *
 * .exec('E57S49', (room) => room.find(FIND_SOURCES));
...
StructureObserver.prototype.watch = function () {
        ...
        if (Game.rooms[watch] && _.get(Game.rooms[watch], 'controller.my', false) === true) {
                Log.info(`Already have visibility in ${watch}. Ending watch operations.`,
if (!(Game.time % 250))
...
StructureObserver.prototype.observeRoom = function (roomName) {
        ...
        if (status === OK) {
                ...
                Log.debug(`Observer ${this.pos.roomName} unable to observe ${roomName} status
${status}`, 'Observer');
        }
        return status;
};
