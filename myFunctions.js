...
global.evaluateEnergyResources = function(creep, ifLink, ifStorage, ifDropped, ifContainer) { // determine which resource to get
    ...
    var energyDropped = false;
      ...
      for (let container of containers) {
          ...
          if (((energySource.energy>0)&&(ifSurrounded(energySource) == null)&&(creep.pos.getRangeTo(energySource)>1))
    ...
    if (ifContainer) { // find energy in links
      var containers = room.find(FIND_MY_STRUCTURES, {filter : c => ((c.structureType == STRUCTURE_CONTAINER)&& (c.pos.findInRange(FIND_HOSTILE_CREEPS, 3).length == 0))});
      for (let container of containers) {
