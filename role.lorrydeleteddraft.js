      ...
      //  ifMineral = mineralNeedsCollect(creep.room);
        ...
        if (creep.memory.working == true) { // if filled with energy, transfer to spawn, extensions or towers
            if (creep.carry.energy>0) { // creep is tranfering energy
                ...
                if (structure == undefined) { // if spawn extensions or towers are full, go find labs
                  var structure = creep.pos.findClosestByRange(FIND_MY_STRUCTURES, { 
                      filter: (s) => ( s.structureType == STRUCTURE_LAB && s.energy < s.energyCapacity*0.75) 
                  
                  || ( s.structureType == STRUCTURE_TERMINAL && (s.store[RESOURCE_ENERGY] < 25000) )
            ...
            if  (creep.ticksToLive<400){
             //   || (ifMineral == undefined)
             //   || (ifMineral[0]<500) ) { // creep is getting energy
